...
class TestUserFunctions(unittest.TestCase):
    ...
    def test_update_user(self, input_mock):
        ...
        input_mock.return_value = TEST_USER_ID
        update_user()
        ...
        file = open(Config.PATH_TO_USERS_FILE, 'r')
        data = json.loads(file.read())
        self.assertEqual(data[0]['first_name'], TEST_USER_ID)
        self.assertEqual(data[0]['last_name'], TEST_USER_ID)
        self.assertEqual(data[0]['Email'], TEST_USER_ID)
        file.close()
        shutil.rmtree(TEST_DATABASE_DIRECTORY)